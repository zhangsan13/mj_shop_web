<template>
  <div class="addDiscount_box">
    ...
    <div class="title_box">
      <ul>
        <li class="active" v-if="pageName === 0">折扣</li>
        <li class="active" v-if="pageName === 1">满折</li>
      </ul>
    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" class="demo-ruleForm">
      ...
      <el-form-item label="活动规则" label-width="100px" v-if="pageName === 1">
        <div class="add_prepend">
          <el-input style="width: 260px;" v-if="pageType !== 'detail'" type="number" v-model.number="fullDiscount.meetPrice">
      ...
      <el-form-item label="活动范围" label-width="100px">
        <div>
          <el-button :type="isAllProduct === 1 ? 'primary' : 'default'" @click="getGoods(1)" :disabled="pageType === 'detail' ? true : false">全店商品</el-button>
          <el-button :type="isAllProduct === 2 ? 'primary' : 'default'" @click="getGoods(2)" :disabled="pageType === 'detail' ? true : false">部分商品
          </el-button>
        </div>
      </el-form-item>
      ...
      <el-form-item>
        ...
        <div class="table table1" style="height: 450px; overflow: auto;" v-if="pageType !== 'detail'">
          <div style="margin-bottom: 10px;" class="add_tips_p" v-if="isAllProduct === 1">选择全店商品时，活动结束前发布的新商品，将会默认加入优惠活动。</div>
          <el-table
            ...
            <el-table-column v-if="pageName === 0"
              ...
              <template slot-scope="scope">
                <div>
                  <el-input type="number" :disabled="isAllProduct === 1 ? true : false" v-model.number="selectGoods[scope.$index].discount" @blur="m3(scope.$index)"></el-input>
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column v-if="pageName === 0"
              ...
              <template slot-scope="scope">
                ...
                <div v-if="scope.row.minPrice === scope.row.maxPrice">
                  <div v-if="selectGoods[scope.$index].discount">{{(selectGoods[scope.$index]['sellPrice'] * selectGoods[scope.$index].discount * 0.1).toFixed(2)}}</div>
                </div>
                <div v-else>
                  <div v-if="selectGoods[scope.$index].discount">{{(scope.row.minPrice * selectGoods[scope.$index].discount * 0.1).toFixed(2)}} - {{(scope.row.maxPrice * selectGoods[scope.$index].discount * 0.1).toFixed(2)}}</div>
                </div>
              </template>
            </el-table-column>
    ...
    <el-dialog
      ...
      <div class="discount_dialog">
        <span>折扣</span>
        <span><el-input placeholder="请输入折扣力度" type="number" v-model.number="ruleForm.discount" @blur="m2"></el-input></span>
      </div>
      <span slot="footer" class="dialog-footer">
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      /*isAllProduct(newValue, oldValue) {
        // alert(newValue);
        if (newValue === 0){
          this.selectGoods = [];
        }
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        // console.log(val, '111111111111111', this.changePage);
        let _flag = false;
        if (this.changePage) {
      ...
      rendering() {
        //渲染
        let _list = this.shopGoodsTable.list;
        let _indexList = new Array();
      ...
      toggleSelection(rows) {
        ...
        if (rows) {
          ...
          rows.forEach(index => {
            // console.log(this.shopGoodsTable.list[index],'111',this.$refs.multipleTable);
            this.$refs.multipleTable.toggleRowSelection(this.shopGoodsTable.list[index]);
          });
        } else {
          this.$refs.multipleTable.clearSelection();
        }
