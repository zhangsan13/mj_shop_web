<template>
  <div class="addDiscount_box">
    ...
    <div class="title_box">
      <ul>
        <li class="active"><span v-if="pageType === 'add'">添加</span><span v-if="pageType === 'edit'">修改</span>赠品信息</li>
      </ul>
            ...
            <ul> <!--拖动调整图片顺序（vue.Draggable ）-->
              ...
              >
                <li v-for="(item, index) in mainPicData" :key="index">
                  <input type="file" @change="uploadSectionFile"/>
                  <img :src="item" alt="" title="点击查看大图" />
                  <span v-if="index === 0 && item!== ''">主图</span>
                  <span v-if="item === ''">上传图片</span>
                </li>
                <!-- <el-upload
        ...
        if(this.pageType=='edit'){      //修改
          let InsertGiftActivityInput={
              ...
              gift: {
                ...
                pics: "http://kuailian-upload.oss-cn-shenzhen.aliyuncs.com/RechargeFile/1558435078042demo-1-bg.jpg,http://kuailian-upload.oss-cn-shenzhen.aliyuncs.com/RechargeFile/15486553518812-5.png,http://kuailian-upload.oss-cn-shenzhen.aliyuncs.com/RechargeFile/15483178873745-7.png"
              }
            }
      ...
      queryGift(){      //查询详情
        ...
        queryGiftProdctInfo.call(this,`?input.type=0&input.id=${id}&input.clientTimespan=${this.clientTimespan}`).then(res=>{
          if (res.data.code===1) {
            ...
            for(let i=0;i<5;i++){
              this.mainPicData.push(imgageInfo[i]);
            }
