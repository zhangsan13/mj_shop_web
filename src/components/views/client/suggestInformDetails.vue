<template>
  <!--建议详情，举报详情-->
  <div class="container">
    <div class="state">{{null == detailData.disposeTime ? '未处理' : '已处理'}}</div>
    <div class="main">
      <div class="example" v-if="null != detailData.disposeTime">
        ...
        <div class="example-right">
          ...
          <ul class="show-img">
            <viewer class="describe-ul-img" :images="imgSrc">
              <li v-for="(item, index) in detailData.disposePics.split(',')" :key="index">
      ...
      <div class="example">
        <span class="iconic-left before"><i></i></span><span class="time">{{detailData.submitTime}}</span><span class="opinion">{{pageType === 0 ? '提交举报' : '提交建议'}}</span>
        <div class="example-right none-border">
          <ul class="show-list" v-if="pageType===0">
            ...
            <li><span class="show-list-left">建议详情：</span><span class="show-list-right">{{detailData.disposeContent}}</span></li>
          </ul>
          <ul class="show-list" v-if="pageType===1">
            ...
            <li><span class="show-list-left">举报详情：</span><span class="show-list-right">{{detailData.disposeContent}}</span></li>
          </ul>
          <ul class="show-img">
            <viewer class="describe-ul-img" :images="imgSrc">
              <li v-for="(item,index) in detailData.submitPics.split(',')" :key="index">
                <img class="papers vue-lazyload avatar" v-lazy="item" alt="" title="点击查看大图">
              </li>
            </viewer>
          </ul>
