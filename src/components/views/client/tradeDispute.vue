<template>
  ...
  <div class="container">
    ...
    <div class="main">
      ...
      <!--筛选-->
      <div class="filtrate">
        <div class="filtrate-search"><span>单    号：</span><el-input style="width: 170px;" v-model="orType" placeholder="请输入单号"></el-input></div>
      ...
      <div class="table-list">
        <el-table :data="tableData" style="width: 100%" :border="true">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <span class="operation" @click="handleProof(scope.$index, scope.row)">去举证</span>
              <!--<span class="operation" @click="handleLook(id,scope.row.orNumber)">查看详情</span>-->
              <router-link :to="{path:'/client/tradeDisputeDispose',query:{'id':id,'orNumber':scope.row.orNumber}}">查看详情</router-link>
            </template>
          </el-table-column>
...
<script>
import Dispute from '../../../api/client/tradeDispute';
export default {
    name: "tradeDispute",
    data(){
        return {
    ...
    methods:{
      ...
      //查询列表函数
        searchBtn(){
            Dispute.call(this,
                '?orType=' +this.orType+
                '&?orTitle='+this.disputeOptions.value+
                '&?orPlate='+this.disposeOptions.value+
                '&pageNum=' + this.currentPage +
                '&?isProof='+this.proofOptions.value+
