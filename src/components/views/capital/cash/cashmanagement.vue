<template>
  <div class="cash">
        ...
        <div class="set_money_left">
          ...
          <div class="set_money_2">
            ...
            <div class="set_money_2_bottom">
              <p class="title">为什么买家付款了，钱不能提现？</p>
    ...
    <div class="recentflow">
      ...
      <p class="rec_right"><a href="">更多流水>></a></p>
      <el-table
        :data="accData"
        border
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <strong v-if="scope.row.update <= 0" style="color: rgb(0, 128, 0)">{{scope.row.update}}</strong>
            <strong v-else style="color: rgb(255, 0, 0)">{{scope.row.update}}</strong>
          </template>
...
<script>
  import GetWithdrawLog from '@/api/Finance/GetWithdrawLog'
  ...
  export default {
    ...
    methods:{
      ...
      getTableData() {
        ...
        GetWithdrawLog.call(this, `?input.pageSize=${this.pageSize}&input.pageIndex=${this.currentPage}&input.clientTimespan=${this.$Tool.formatDate(new Date())}`).then(res => {
