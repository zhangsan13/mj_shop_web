    ...
    <div class="main">
      ...
      <ul class="parent-ul">
        <li v-for="(item,index) in tableData" :key="item.id">
          <div class="parent"><div  @click="show(index)">{{item.title}}</div>
            <div v-show="!isAdjustSort">
              <span @click="handleCompile(1,index,'',item.id)">编辑</span>
              <span @click="handleDelete(item.id,item.parentId)">删除</span></div>
          ...
          <ul v-show="isShow[index]">
            <li v-for="(items,indexs) in item.childCategory" :key="items.id">
              <div class="child"><div><span>{{items.title}}</span></div>
                <div v-show="!isAdjustSort">
                  <span @click="handleCompile(2,index,indexs,item.id)">编辑</span>
                  <span @click="handleDelete(items.id,items.parentId)">删除</span></div>
