    ...
    <div class="warp_bg">
      ...
      <div class="screen">
        <div class="left">
          ...
          <ul>
            <li v-for="(item, i) in cateFilter(firstCateData, search_first)"  :key="i" @click="firstCateActive = item.cid;" :class=" firstCateActive == item.cid ? 'cur' : '' ">{{item.title}}<i class="iconfont icon-right"></i> </li>
          </ul>
        </div>
        <div class="middle">
          <div class="search">
          ...
          <ul>
            <li v-for="(item,i) in cateFilter(secondCateData, search_second)" :key="i" @click="secondCateActive = item.cid; secondCateTitle = item.title;" :class=" secondCateActive == item.cid ? 'cur' : '' ">{{item.title}}<i class="iconfont icon-right"></i> </li>
          </ul>
        </div>
        <div class="right">
          <div class="search">
        ...
        <div class="search_box" v-if="searchCateVisible">
          <div class="search_top">
            ...
            <i class="el-icon-error" @click="searchCateVisible = false"></i>
          </div>
      ...
      <div class="authent">
        ...
        <div>
          <label>商家资质：</label><img src="./img/renz_not.png" alt=""> <span>商家当前未完成当前类目商品资质认证，前去认证</span>
        </div>
      </div>
      <!--商家认证结束-->
  ...
  export default {
    ...
    methods: {
      ...
      latelyCateChange( e ) {
        ...
        this.firstCateActive = cateSelected[0];
        this.secondCateActive = cateSelected[1];
        this.thirdCateActive = cateSelected[2];
      },
        ...
        GetGoodsPublishCategory.call(this, `?clientTimespan=${clientTimespan}&parentId=${parentId}`).then( res => {
          ...
          if (res.data.code === 1) {
            if ( level === 'firstCate' ) {
              ...
              if (this.secondCateData.filter(val => val.cid == this.secondCateActive)[0]) {
                this.secondCateTitle = this.secondCateData.filter(val => val.cid == this.secondCateActive)[0].title;
              }
            } else if ( level === 'thirdCate' ) {
              this.thirdCateData = res.data.data;
              if (this.thirdCateData.filter(val => val.cid == this.thirdCateActive)[0]) {
                this.thirdCateTitle = this.thirdCateData.filter(val => val.cid == this.thirdCateActive)[0].title;
              }
            }
          } else {
            this.$message.error(res.data.desc);
    ...
    watch: {
      firstCateActive( val ) {
        this.firstCateTitle = this.firstCateData.filter(res => res.cid == val)[0].title;
        this.thirdCateData= [];
