    ...
    <div class="box">
      ...
      <div class="right">
        <div class="search">
          ...
          <button class="seaerch_btn">查询</button>
        </div>
        <div class="warp_tree">
          <el-tree
            :data="tree"
    ...
    mounted: function (){
      ...
      shopBusinessScope.call(this,`?input.clientTimespan=${nowTime}`).then(res => {
        if (res.data.code === 1) {
          ...
          let obj = {};
          //将接口数据转换成需要的数据结构
          for(var parent_index in res.data.data){
            for(var child_index in res.data.data[parent_index].childScope){
              this.defaultCheckedKey.push(res.data.data[parent_index].childScope[child_index].id);
              obj.id = res.data.data[parent_index].childScope[child_index].id;
              obj.title = res.data.data[parent_index].childScope[child_index].title;
              obj.parentTitle = res.data.data[parent_index].title;
              obj.parentId = res.data.data[parent_index].id;
              tableDatas.push(obj);
              obj = {};

              this.oldData.push(res.data.data[parent_index].childScope[child_index].id);
            }
          }
